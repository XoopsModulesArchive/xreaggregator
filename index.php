...
if(isset($_GET['id'])||isset($_GET['xreaggregator'])) {
	if (isset($_GET['xreaggregator'])){
		...
		$cid=intval($_GET['cid']);
		$sql = "SELECT a.xreaggregator_name FROM ".$GLOBALS['xoopsDB']->prefix('xreaggregator')." a where a.xreaggregator_id = $storytopic";
		$ret = $GLOBALS['xoopsDB']->query($sql);
		$row = $GLOBALS['xoopsDB']->fetchArray($ret);
		header( "HTTP/1.1 301 Moved Permanently" ); 
		header( "Location: ".XOOPS_URL."/" . $GLOBALS['xoopsModuleConfig']['baseurl'] . "/".xoops_sef($row['xreaggregator_name'])."/$storytopic,$cid".$GLOBALS['xoopsModuleConfig']['endofurl']);
		exit(0);
	}
} else {
	if(isset($_GET['id'])) {
		...
		if (strpos(' '.$_SERVER['REQUEST_URI'],'odules/')>0){
			header( "HTTP/1.1 301 Moved Permanently" ); 
			header( "Location: ".XOOPS_URL."/" . $GLOBALS['xoopsModuleConfig']['baseurl'] . "/");
			exit(0);
		}
	}
}
