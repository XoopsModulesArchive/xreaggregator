	...
	if(isset($_GET['mid'])||isset($_GET['mashable'])) {
		if (isset($_GET['mashable'])){
			...
			if (strpos($_SERVER['REQUEST_URI'],'odules/')>0)
			{
				...
				header( "Location: ".XOOPS_URL."/" . $GLOBALS['xoopsModuleConfig']['baseurl'] . "/".xoops_sef($row['name'])."/mashable,$mashtopic" . $GLOBALS['xoopsModuleConfig']['endofurl_rss']);
				exit(0);
			}	
		}
		...
		if (strpos($_SERVER['REQUEST_URI'],'odules/')>0)
		{
			...
			header( "HTTP/1.1 301 Moved Permanently" ); 
			header( "Location: ".XOOPS_URL."/" . $GLOBALS['xoopsModuleConfig']['baseurl'] . "/feed,cat,$cid" . $GLOBALS['xoopsModuleConfig']['endofurl_rss']);
			exit(0);
		}
		...
		if (isset($_GET['xreaggregator'])){
			...
			if (strpos($_SERVER['REQUEST_URI'],'odules/')>0)
			{
				...
				header( "HTTP/1.1 301 Moved Permanently" ); 
				header( "Location: ".XOOPS_URL."/" . $GLOBALS['xoopsModuleConfig']['baseurl'] . "/".xoops_sef($row['xreaggregator_name'])."/feed,$storytopic" . $GLOBALS['xoopsModuleConfig']['endofurl_rss']);
				exit(0);
			}	
		}
	} else {
		if(isset($_GET['id'])) {
			...
			if (strpos($_SERVER['REQUEST_URI'],'odules/')>0)
			{
				...
				if (strlen($row['xreaggregator_name'])>0)
				{
					...
					header( "Location: ".XOOPS_URL."/" . $GLOBALS['xoopsModuleConfig']['baseurl'] . "/feed,$storytopic,".$row['xreaggregator_cid']."" . $GLOBALS['xoopsModuleConfig']['endofurl_rss']);
					exit(0);
				}
			}
			...
			if (strpos($_SERVER['REQUEST_URI'],'odules/')>0)
			{
				if (strpos(' '.$_SERVER['REQUEST_URI'],'odules/')>0){
					header( "HTTP/1.1 301 Moved Permanently" ); 
					header( "Location: ".XOOPS_URL."/" . $GLOBALS['xoopsModuleConfig']['baseurl'] . "/feed" . $GLOBALS['xoopsModuleConfig']['endofurl_rss']);
					exit(0);
				}
			}
		}
...
if (!$tpl->is_cached('db:xreaggregator_rss.html', $xoopsCachedTemplateId, $compile_id)) {
	...
	if ($hlid > 0) {
		...
		if (is_a($ml, 'xreaggregatorMashables'))
			$channel['title'] = $GLOBALS['xoopsConfig']['sitename'].' :: '.$ml->getVar('name').' :: RSS Mashable';
		else
			$channel['title'] = $GLOBALS['xoopsConfig']['sitename'].' :: RSS Re-aggregation';
		$channel['link'] = XOOPS_URL;
		$channel['description'] = $GLOBALS['xoopsConfig']['slogan'];
		...
		$image['title'] = $GLOBALS['xoopsConfig']['sitename'].' :: RSS Re-aggregation';
